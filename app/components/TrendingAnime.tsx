...
const TrendingAnime = ({
  ...
  const { user } = useAuth();
  const { trendingAnime, loading, fetchTrendingAnime } = useAnimeData();
  ...
  const loadAnime = useCallback(async () => {
    try {
      ...
      const limitedAnime = maxItems
        ? trendingAnime.slice(0, maxItems)
        : trendingAnime;
  ...
  }, [fetchTrendingAnime, trendingAnime, maxItems]);
    ...
    <View style={styles.container}>
      {showHeader && (
        <View style={styles.headerContainer}>
          <Text style={styles.headerTitle}>Trending Anime</Text>
          {onViewAllPress && (
